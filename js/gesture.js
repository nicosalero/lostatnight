 /**
	...
	/**
		...
		/* Obtener el tamano del contenedor */
		var iframe = document.getElementById('iframe-container');
		var style = window.getComputedStyle(iframe);
		/* Variables String*/
		var containerWidthS = style.getPropertyValue('width'); // 684px
		var containerHeightS = style.getPropertyValue('height');
		/* Obtener el numero de pixeles */
		var containerWidth = parseInt(containerWidthS, 10);	// En pixeles- number: 684
		var containerHeight = parseInt(containerHeightS, 10);	// En pixeles- number
		// point = pixel * 72 / 96

		// volver a calcular cada vez que se redimensiona la ventana
		$(window).resize(function(){
			iframe = document.getElementById('iframe-container');
			style = window.getComputedStyle(iframe);
			containerWidthS = style.getPropertyValue('width');
			containerHeightS = style.getPropertyValue('height'); // 684px
			containerWidth = parseInt(containerWidthS, 10);	// In pixeles- number
			containerHeight = parseInt(containerHeightS, 10);	// In pixeles- number
			});
